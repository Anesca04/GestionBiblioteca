...
public class VerLibrosPrestados extends JPanel{
    private JTextField txCodigo;
    private JTextField txTitulo;
    private JTextField txEditorial;
    private JTextField txPaginas;

    public VerLibrosPrestados(Ventana v) {
        setLayout(null);

        JLabel lbPrincipal = new JLabel("Libros prestados");
        ...
        int numLibros = v.cliente.getLibrosPrestados().size();
        
        textArea.append("\n");
        for (int i = 0; i < numLibros; i++){
            Libro libroPrestado = v.cliente.getLibrosPrestados().get(i);
            Prestamo prestamo = v.biblioteca.getPrestamo(v.cliente, libroPrestado);
            textArea.append(" " + (i+1) + ". " + libroPrestado.toString() + "\n");
            if (prestamo != null){
                textArea.append("\t- Fecha préstamo: " + prestamo.getFechaPrestamo() + "\n");
                textArea.append("\t- Fecha límite de devolución: " + prestamo.getFechaDevolucion() + "\n");
            }
            
        }
        
        if (numLibros == 0){
            textArea.append(" No tienes ningún libro prestado");
        }
